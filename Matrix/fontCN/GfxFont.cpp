...
{
	  0,  4,  8, 12, 24, 20, 24, 28, 48, 36, 40, 44, 48,
...
void GfxFont::Print(int upColor, int downColor, float x, float y, const char *format, ... )
{
	char sBuffer[10240] = {0};
	char *lpsArg=(char*)&format+sizeof(format);
	vsprintf(sBuffer, format, lpsArg);

	Render(upColor,downColor,x,y,CA2W(sBuffer));
}

void GfxFont::Render(int upColor, int downColor, float x, float y, const wchar_t* text )
{
	float offsetX = x;
	float offsetY = y;
	...
	while(*text)
	{
		...
		else
		{
			...
			if (idx > 0)
			{
				...
				m_pSprite->SetTexture(m_Glyphs[idx].t);
				m_pSprite->SetTextureRect(0, 0, m_Glyphs[idx].w, m_Glyphs[idx].h);
				m_pSprite->Render(offsetX - m_Glyphs[idx].x + 1, offsetY - m_Glyphs[idx].y + 1);
				SetColor(upColor);
				m_pSprite->SetTexture(m_Glyphs[idx].t);
				m_pSprite->SetTextureRect(0, 0, m_Glyphs[idx].w, m_Glyphs[idx].h);
				m_pSprite->Render(offsetX - m_Glyphs[idx].x, offsetY - m_Glyphs[idx].y);
	...
	if (idx < font_count && NULL == m_Glyphs[idx].t)
	{
		...
		if((signed)nLen > 0)
		{
			...
			if (nLen == ::GetGlyphOutlineW(m_hMemDC,nChar,m_nAntialias,&gm,nLen,lpBuf,&mat2))
			{
				...
				{
					LONG nSrcPitch = (gm.gmBlackBoxX / 48 + (gm.gmBlackBoxX % 48 == 0 ? 0 : 1)) * 4;
